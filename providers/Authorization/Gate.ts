...
export default class Gate {
  private actions = new Map<string, {callback: GateCallback; definition: GateDefinition}>()
  private policies = new Map<any, { name: any; instance: any; }>()

  public define (action: string, callback: GateCallback, options?: GateOptions) {
    this.actions.set(action, { callback, definition: new GateDefinition(options) })
  }
  ...
  public async allowsAction (
    ...
    throw new Error(
      `Found no action callback for action ${action}`,
    )
  }

  public async allowsResource (user: unknown, action: string, resource: any, ...params: unknown[]): Promise<boolean> {
    let constructor
    ...
    if (policyData) {
      ...
      if (!policyMethodData) {
        throw new Error(`Found no policy gate named ${action} on policy ${policyData.name}`)
      }
...
class UserGate {
  constructor (private user: unknown, private gate: Gate) {
    this.gate = gate
    this.user = user
  }
...
class UserGateWithResource {
  constructor (private user: unknown,private gate: Gate, private resource: unknown){}

  public allows (action: string, ...params: unknown[]): Promise<boolean> {
    return this.gate.allowsResource(this.user, action, this.resource, ...params)
  }
